...
<template>
  <ChartContainer :config="chartConfig" class="h-80 px-4 touch-pan-y">
      ...
      <div class="flex items-center justify-center h-64">
        <div class="text-center text-muted-foreground">
          <Icon
            ...
            class="h-12 w-12 mx-auto mb-2 animate-spin opacity-50"
          />
    ...
    <template v-else>
      ...
      >
        <VisLine
          v-for="(fund, index) in props.funds"
          :key="fund.schemeCode"
          :x="(d: ChartDataPoint) => d.date"
          :y="(d: ChartDataPoint) => d[`${fund.schemeCode}_percentage`]"
          :color="Colors.get(index)"
